...
class GeminiService:
    def __init__(self):
        ...
        if not api_key:
            print("GEMINI_API_KEY non définie. Service Gemini indisponible.")
    ...
    def translate_text(self, text: str, target_language: str) -> Optional[str]:
        ...
        if not self.is_available:
            print("DEBUG: GeminiService non disponible, skipping translation.") # Ajout d'un log
            return None
    ...
    def _build_translation_prompt(self, text: str, target_language: str) -> str:
        ...
        # Amélioration du prompt pour plus de clarté et de robustesse
        # Ajout d'une instruction pour le format de sortie
        prompt = f"""
Tu es un expert en traduction vers les langues africaines locales.
Ton objectif est de traduire précisément des phrases courtes du français vers le {target_language}.
